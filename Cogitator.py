...
import argparse
import statistics
from outStruct import CanticleOutput, BroadcastOutput, StatsOutput
...
parser.add_argument("-f", action="store_true", help="select if the Cogitator should be displayed full screen (without the 1 pixel line around). Mutually exclusive with background transparency", 
                    dest="fullscreen")
parser.add_argument("-s", action="store_false", help="turn on miscellaneous sound effects",
                    dest="sound")
parser.add_argument("-p", action="store_true", help="turn on the constant praise to the Omnissiah",
                    dest="praise")
parser.add_argument("-v", action="store_const", const=10, help="set the volume of sound and praise effects",
                    dest="volume")
parser.add_argument("-m", default=0, dest="monitor", nargs='?', const=0, type=int,
                    help="""select screen index on which Cogitator should be displayed. 
                    If selected index is out of range, the default (main) monitor will be used""")
...
screen_width = pr.get_monitor_width(settings["monitor"])
...
# initialize Stats
statOut = StatsOutput(color, screen_height, screen_width, output_font_size,
                      broadcast_font, noosphere_font, settings)
...
while not pr.window_should_close():
    FPS_counter += 1
    frames.append(pr.get_frame_time())
    ...
    if init_flag == False:
        ...
        statOut.draw_stat_output(frame_avg)
        
        if FPS_counter % 60 == 0:
            frame_avg = statistics.fmean(frames)
            frames = []
